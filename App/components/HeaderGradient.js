...
const HeaderGradient = ({navigation, children, absolute=true, closeX = false, title=""}) => {
    return <View style={{width: '100%', height: absolute?90:constants.HEADERHEIGHT, position:absolute?'absolute':'relative', top: 0, zIndex: 500,backgroundColor: constants.TRANSLUCENT, borderBottomRightRadius: 30, borderBottomLeftRadius: 30}}>
          <LinearGradient
    ...
    style={{
      ...
      //alignItems: 'center',
        ...
        <View style={{zIndex: 10, position: 'absolute', bottom: 20, right: 0, width: '100%'}}><Text style={{fontFamily: constants.FONT, alignSelf: 'center', fontSize: 18,}}>{title}</Text></View>
        {children}
